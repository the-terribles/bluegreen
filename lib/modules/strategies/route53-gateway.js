...
class Route53GatewayStrategy extends Strategy {
  ...
  static transformGateway(record){
    let gateway = {
      ...
      uri: record.Name.substring(0, record.Name.length - 1),
  ...
  static mergeTXTandARecords(records){
    ...
    txtRecords.forEach((TXT) => {
      let A = _.find(records, (R) => {
        return R.Type === 'A' && TXT.Name === R.Name;
      });
      if (A) TXT.A = A;
    });
    ...
    this.listGateways(bluegreen, context, (err, gateways) => {
      ...
      async.waterfall([
        ...
        (HostedZoneId, HostZoneName, next) => {
          ...
          let TXT = {
            Name: `${rrname}.${HostZoneName}`,
            Type: 'TXT',
            TTL: 0,
            ResourceRecords: [
              {
                Value: Route53GatewayStrategy.hydrateMetadata(context.gateway, context.service, null)
              }
            ]
          };
          ...
          console.log('Trying to create TXT record', JSON.stringify(request, null, 2));
