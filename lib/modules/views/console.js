...
class ConsoleViewLayer {
  ...
  renderHelp(bluebird, commands){
    ...
    buffer += 'A simplified workflow for BlueGreen deployments...\n';
    ...
    _.keys(groups).forEach((group) => {
      ...
      t.cell('Usage', colors.yellow(group.toUpperCase()));
      t.newRow();

      groups[group].forEach((command) => {

        t.cell('Usage', colors.blue('bluegreen ') + colors.green(ConsoleViewLayer.formatCommandPlaceholders(command.usage)));
