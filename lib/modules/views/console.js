...
class ConsoleViewLayer {
  ...
  render(bluebird, layout, data){
    switch(layout){
      ...
      case Views.Services : return this.renderServices(data);
  ...
  renderServices(services){
    ...
    if (!services || services.length === 0){
      return console.log(colors.yellow('No services are registered.'));
    }

    console.log(colors.magenta(`\nServices managed by BlueGreen\n`));

    let t = new Table();

    services.forEach((service) => {

      t.cell('Name', colors.blue(service.name));
      t.cell('Created', moment(service.created).fromNow());
      t.cell('Updated', moment(service.updated).fromNow());
      t.newRow();
    });
